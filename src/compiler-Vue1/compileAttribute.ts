 ...
 * @param {*} vm Vue 实例
 */
export default function compileAttribute(node:any, vm:any) {
  // 将类数组格式的属性节点转换为数组
  let attrs: any[]
 ...
 * @param {*} vm Vue 实例
 */
function compileVOnClick(node:any, method:string, vm:any) {
  // 给节点添加一个 click 事件，回调函数是对应的 method
  node.addEventListener('click', function (...args: any[]) {
 ...
 * @param {*} vm Vue 实例
 */
function compileVBind(node:any, attrValue:string, vm:any) {
  // 属性名称
 ...
 * @param {*} vm Vue 实例
 */
function compileVModel(node:any, key:string, vm:any) {
  // 节点标签名、类型
